...
let FATHOM_VERSION = "v0.2.20-Beta"
...
final class ProcessEnergyStore: ObservableObject {
    ...
    /// Skip path lookups when UI closed — name is enough for drain log.
    private var lastTopLog: CFAbsoluteTime = 0

    func start() {
        restartTimer(interval: fastMode ? kProcPollUI : kProcPollIdle)
        // Defer first sample so launch stays light
    ...
    private func sample() {
        ...
        workQ.async { [weak self] in
            ...
            // Smaller PID buffer + early filter — full taskallinfo on 400+ pids is expensive
            ...
            // Raise skip threshold when idle to avoid name/path work
            ...
            // Drop stale history for dead pids
            ...
            DispatchQueue.main.async {
                // Avoid thrashing SwiftUI when ranking unchanged
...
// MARK: - Menubar popover (BatFi-style)

/// Compact BatFi-like battery menu. "Why drain?" is pinned to the bottom.
struct MainRoot: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // Live watts strip (plain language)
            ...
            // Power-draw sparkline (~1h history when warm)
            ...
            // ── Why did battery drop? pinned bottom ──
            MenuHairline()
...
enum MenuBarBatteryIcon {
    ...
    /// Single template image: battery glyph + time remaining (or W / % fallback).
    static func image(
        level: Int,
        charging: Bool,
        onAC: Bool,
        ...
        // Smaller font when showing "87% 2h 15m"
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Snappier open/close than default popover animation
        pop.animates = false
        pop.delegate = self
        // Match dark menu surface
        pop.appearance = NSAppearance(named: .darkAqua)
        popover = pop
        // Warm the hosting controller so first click is instant
        ...
        // Backup menubar refresh only — Combine sinks handle live changes; keep light
    ...
    private func updateStatusTitle() {
        ...
        // Throttle composite redraws: only when visible label/state changes
    ...
    private func attachPopoverContentIfNeeded() {
        ...
        // Keep MainRoot warm — no animation so live metrics feel immediate
        let root = AnyView(
            MainRoot()
