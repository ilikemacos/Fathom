...
#
# v0.1.0-Beta — Live power dashboard, process energy ranking, drain attribution.
#
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
# Apple Silicon preferred; Intel allowed as best-effort
ARCH="$(uname -m)"
...
# Integrity: mask EXPECTED_HASH line then hash whole script (same pattern as rNitro)
EXPECTED_HASH="9de1982223f05fec6877055c7baf55ae2150e5cc0f656b98f25b9224c0c6ad54"
...
let FATHOM_VERSION = "v0.1.0-Beta"
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        PowerStore.shared.$packageWatts
            .combineLatest(PowerStore.shared.$levelPercent, PowerStore.shared.$batteryPresent)
            .receive(on: DispatchQueue.main)
            .sink { [weak self] w, level, present in
                self?.updateTitle(watts: w, level: level, present: present)
            }
            .store(in: &bag)

        updateTitle(watts: 0, level: 0, present: false)
    }

    private func updateTitle(watts: Double, level: Int, present: Bool) {
        guard let button = statusItem?.button else { return }
        if present {
            button.title = String(format: " %.0fW %d%%", watts, level)
        } else {
            button.title = String(format: " %.1fW", watts)
        }
        button.toolTip = "Fathom — local battery drain"
    }
