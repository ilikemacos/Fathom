#!/bin/bash
# Fathom installer v0.2.6-Beta — IOReport package watts + sleep/wake drain events
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="8bc8698e60cde827dac3463d1c99896ebe816966e78ec2b9bdea19d22523ab4e"
...
let FATHOM_VERSION = "v0.2.6-Beta"
...
// MARK: - Battery menu (stock Battery + BatFi-style)
...
struct MainRoot: View {
    ...
    @State private var why = DrainLog.shared.whyLastHour()
    @State private var showFathomSettings = false
    @State private var retentionDays: Double = Double(DrainLog.shared.retentionDays)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(alignment: .center, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(power.batteryPresent ? "\(power.levelPercent)%" : "AC")
                        .font(uiFont(28, weight: .semibold))
                        .foregroundColor(.labelPrimary)
                        .monospacedDigit()
                    Text(statusDetail)
                        .font(uiFont(12, weight: .regular))
                        .foregroundColor(.labelSecondary)
                        .lineLimit(2)
                    if power.packageWatts > 0.05 {
                        Text(String(format: "Package  %.1f W", power.packageWatts))
                            .font(uiFont(12, weight: .medium))
                            .foregroundColor(.labelTertiary)
                            .padding(.top, 1)
                    }
            ...
            MenuHairline().padding(.horizontal, 16)

            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 0) {
                    // Power Source (stock Battery menu)
                    MenuSectionHeader(title: "Power Source")
                        .padding(.horizontal, 16)
                    Text(powerSourceLine)
                        .font(uiFont(13, weight: .regular))
                        .foregroundColor(.labelPrimary)
                        .padding(.horizontal, 16)
                        .padding(.bottom, 10)
                    ...
                    Text(lowPower
                         ...
                         : "Turn on in Control Center or Battery Settings.")
                        .font(uiFont(11, weight: .regular))
                        .foregroundColor(.labelTertiary)
                        .padding(.horizontal, 16)
                        .padding(.bottom, 10)
                    ...
                    // Apps Using Energy (stock wording)
                    ...
                    // Why drain? (Fathom)
                    MenuSectionHeader(title: "Why Drain?")
                        .padding(.horizontal, 16)
                    Text(why.summary)
                        .font(uiFont(13, weight: .regular))
                        .foregroundColor(.labelPrimary)
                        ...
                        .padding(.bottom, 4)
                    if why.dropPercent > 0 {
                        Text("~\(why.dropPercent)% over \(why.windowLabel)")
                            .font(uiFont(12, weight: .regular))
                            .foregroundColor(.labelSecondary)
                            .padding(.horizontal, 16)
                    }
                    ForEach(Array(why.culprits.prefix(3).enumerated()), id: \.offset) { _, c in
                        HStack {
                            Text(c.name)
                                .font(uiFont(12, weight: .regular))
                                .foregroundColor(.labelSecondary)
                    ...
                    // Footer actions (stock Battery menu pattern)
                    MenuRowButton(title: "Battery Settings…") {
                    ...
                    Toggle(isOn: Binding(
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Replace system Battery icon")
                                .font(uiFont(13, weight: .regular))
                                .foregroundColor(.labelPrimary)
                            Text("Hides Apple’s Battery menu item while Fathom runs.")
                                .font(uiFont(11, weight: .regular))
                                .foregroundColor(.labelTertiary)
                        }
                    }
                    ...
                    MenuSectionHeader(title: "Menu Bar Position")
                        .padding(.horizontal, 16)
                    Text("Hold ⌘ and drag the Fathom battery icon in the menu bar to move it freely.")
                        .font(uiFont(11, weight: .regular))
                        .foregroundColor(.labelTertiary)
                        .fixedSize(horizontal: false, vertical: true)
                        .padding(.horizontal, 16)
                    ...
                    .padding(.bottom, 4)
                    HStack(spacing: 8) {
                        MinimalButton(title: "Near clock") { MenuBarPositionStore.normalized = 0.08 }
                        MinimalButton(title: "Middle") { MenuBarPositionStore.normalized = 0.45 }
                        MinimalButton(title: "Far left") { MenuBarPositionStore.normalized = 0.9 }
                    }
                    .padding(.horizontal, 16)
                    .padding(.bottom, 10)

                    MenuRowButton(title: showFathomSettings ? "Hide Fathom Details" : "Fathom Details") {
                        withAnimation(.easeInOut(duration: 0.15)) {
                            showFathomSettings.toggle()
                        }
                    }
                    .padding(.horizontal, 16)
                    .padding(.bottom, showFathomSettings ? 0 : 8)

                    if showFathomSettings {
                        VStack(alignment: .leading, spacing: 6) {
                            MonitorRow(label: "Version", value: UpdateChecker.displayLabel(FATHOM_VERSION))
                            MonitorRow(label: "Channel", value: FATHOM_CHANNEL)
                            MonitorRow(label: "Install", value: UpdateChecker.installPathLabel())
                            HStack(spacing: 8) {
                                MinimalButton(title: "Check for Updates") { UpdateChecker.checkManually() }
                            ...
                            Text("History: \(Int(retentionDays)) days")
                                .font(uiFont(12))
                                .foregroundColor(.labelSecondary)
                            Slider(value: $retentionDays, in: 1...30, step: 1)
                                .tint(Color.white.opacity(0.5))
                                .onChange(of: retentionDays) { _, v in
                                    DrainLog.shared.setRetention(Int(v))
                                }
                            Text("Fathom cannot inject into Apple’s Battery control. It hides that icon and shows its own battery menu instead. Quitting Fathom restores the system icon.")
                                .font(uiFont(11))
                                .foregroundColor(.labelTertiary)
                                .fixedSize(horizontal: false, vertical: true)
                        }
...
enum MenuBarBatteryIcon {
    static func image(level: Int, charging: Bool, onAC: Bool, present: Bool) -> NSImage? {
        let name = BatteryMenuChrome.batterySymbol(
            level: level, charging: charging, onAC: onAC, present: present
        )
        let config = NSImage.SymbolConfiguration(pointSize: 14, weight: .medium)
        guard let img = NSImage(systemSymbolName: name, accessibilityDescription: "Battery")?
    ...
    static func title(level: Int, present: Bool, onAC: Bool, charging: Bool) -> String {
        ...
        if charging { return " \(level)%" }
        if onAC && level >= 100 { return " \(level)%" }
        return " \(level)%"
    }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    private let popoverSize = NSSize(width: 300, height: 520)
    ...
    private func updateStatusTitle() {
        ...
        button.image = MenuBarBatteryIcon.image(
            level: p.levelPercent,
            charging: p.isCharging,
            onAC: p.isOnAC,
            present: p.batteryPresent
        )
        button.title = MenuBarBatteryIcon.title(
            level: p.levelPercent,
            present: p.batteryPresent,
            onAC: p.isOnAC,
            charging: p.isCharging
        ...
        button.toolTip = p.batteryPresent
            ? "Fathom · \(p.levelPercent)% · \(String(format: "%.1f W", p.packageWatts))"
            : "Fathom · AC"
    }
    ...
    private func attachPopoverContentIfNeeded() {
        ...
        hosting.preferredContentSize = NSSize(width: 300, height: 520)
